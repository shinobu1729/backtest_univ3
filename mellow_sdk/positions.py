...
class BiCurrencyPosition(AbstractPosition):
    ...
    def rebalance(self, x_fraction: float, y_fraction: float, price: float) -> None:
        ...
        assert (
            self.x > 1e-6 or self.y > 1e-6
        ), f"Cant rebalance empty portfolio x={self.x}, y={self.y}"
        assert (
            abs(x_fraction + y_fraction - 1) <= 1e-6
...
class UniV3Position(AbstractPosition):
    ...
    def mint(self, x: float, y: float, price: float) -> None:
        ...
        is_optimal, x_liq, y_liq = self.aligner.check_xy_is_optimal(
            x=x, y=y, price=price
        )
    ...
    def burn(self, liq: float, price: float) -> Tuple[float, float]:
        ...
        assert (
            liq <= self.liquidity
        ), f"Too much liquidity too withdraw = {liq} / {self.liquidity}"
    ...
    def snapshot(
        ...
        """
        x, y = self.to_xy(price)
        il_to_x, il_to_y = self.impermanent_loss_to_x(
            price
        ), self.impermanent_loss_to_y(price)
