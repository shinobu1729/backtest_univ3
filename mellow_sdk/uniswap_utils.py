...
class UniswapLiquidityAligner:
    ...
    def price_to_sqrtp(self, price: float) -> float:
        return int(np.sqrt(price) * self.q96)
    ...
    def x_to_liq(self, price, x):
        ...
        sqrt_lower = self.price_to_sqrtp(price=self.lower_price)
        sqrt_upper = self.price_to_sqrtp(self.upper_price)
        sqrt_price = self.price_to_sqrtp(price)
        ...
        return (x * (sqrt_upper * left_bound) / self.q96) / (sqrt_upper - left_bound)
    ...
    def y_to_liq(self, price, y):
        ...
        sqrt_lower = self.price_to_sqrtp(self.lower_price)
        sqrt_upper = self.price_to_sqrtp(self.upper_price)
        sqrt_price = self.price_to_sqrtp(price)
        ...
        return y * self.q96 / (right_bound - sqrt_lower)
    ...
    def liq_to_x(self, price, liq):
        ...
        return liq * self.q96 * (sqrt_upper - left_bound) / (sqrt_upper * left_bound)
    ...
    def liq_to_y(self, price, liq):
        ...
        right_bound = min(sqrt_price, sqrt_upper)
        return liq * (right_bound - sqrt_lower) / self.q96
    ...
    def check_xy_is_optimal(self, price, x, y):
        ...
        digits: int = len(str(round(liq_x)))
        effective_digits = digits - 6
        effective_diff = eval('1e'+str(effective_digits))

        return abs(liq_x - liq_y) < effective_diff, liq_x, liq_y
    ...
    def get_amounts_for_swap_to_optimal(
        ...
        if liq_x > liq_y:
            ...
            den = self.x_to_liq(price=price, x=1.0) + self.y_to_liq(
                price=price, y=(1 - swap_fee) * price
            )
            return num / den, 0

        if liq_x < liq_y:
            num = self.y_to_liq(price=price, y=y) - self.x_to_liq(price=price, x=x)
            den = self.x_to_liq(price=price, x=(1 - swap_fee) / price) + self.y_to_liq(
                price=price, y=1.0
            )
            return 0, num / den
